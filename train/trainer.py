...
class TrainerConfig:
    ...
    # === Mixed precision ===
    use_amp: bool = False              # Enable for faster GPU training
...
class Trainer:
    ...
    def __init__(
        ...
        # ============================================================
        # Optimizer: separate LRs for backbone and controller
        # ============================================================
    ...
    def fit(self) -> None:
        ...
        for epoch in range(self.current_epoch, self.tcfg.num_epochs):
            ...
            if (epoch + 1) % self.tcfg.val_every_n_epochs == 0:
                ...
                # Save best
                if self.tcfg.save_best and val_metrics['total_loss'] < self.best_val_loss:
            ...
            if (epoch + 1) % self.tcfg.ood_eval_every_n_epochs == 0:
                self.evaluate_ood(epoch)
            
            # Periodic checkpoint
            if (epoch + 1) % self.tcfg.save_every_n_epochs == 0:
    ...
    def train_step(self, batch: BatchedSample) -> Dict[str, float]:
        ...
        with autocast(enabled=self.tcfg.use_amp):
            # ============================================================
            # 1. Backbone forward pass (batched)
            # ============================================================
            backbone_out = self.backbone(images, boxes, box_batch_indices)
            node_embeddings_all = backbone_out['node_embeddings']  # [N_total, vis_dim]
            cls_tokens = backbone_out['cls_token']                 # [B, vis_dim]
            heatmap_logits = backbone_out.get('heatmap_logits')    # [B, 1, H/8, W/8]
            
            # ============================================================
            # 2. Heatmap detection loss
            # ============================================================
            heatmap_loss = torch.tensor(0.0, device=self.device)
            if heatmap_logits is not None:
                heatmap_loss = HeatmapHead.focal_loss(
                    heatmap_logits, heatmap_targets
                )
            
            # ============================================================
            # 3. Controller forward pass (per-sample)
            # ============================================================
            ...
            for i in range(B):
                # Unpad graph for this sample
                sample_data = unpad_graph(batch, i, device=self.device)
                graph = sample_data['graph']
                gt_sequence = sample_data['gt_sequence']
                
                # Attach node embeddings from backbone
                box_mask = (box_batch_indices == i)
                graph.node_embeddings = node_embeddings_all[box_mask]  # [N_i, vis_dim]
                
                # CLS token for this sample
                cls_token_i = cls_tokens[i]  # [vis_dim]
                
                # Run controller with teacher forcing
            ...
            # Average controller losses over batch
        ...
        # Optimizer step (respecting accumulation)
    ...
    def evaluate_ood(self, epoch: int) -> Dict[int, Dict]:
        ...
        that exceed the training distribution.
        """
        self.backbone.eval()
        self.controller.eval()
        ...
        for length, loader in ood_loaders.items():
            ...
            for batch in loader:
                ...
                output = self.controller.forward_inference(
                    ...
                    cls_token=cls_token,
                    device=self.device,
                    max_steps=length * 3,  # Generous step budget
                    greedy=True
                )
                
                # Evaluate
                gt_tokens = [t['token'] for t in sample_data['gt_sequence']]
                pred_tokens = output.predicted_sequence
                
                # Remove <END> token for comparison
                pred_tokens = [t for t in pred_tokens if t != '<END>']
    ...
    def save_checkpoint(self, tag: str) -> str:
        ...
        path = os.path.join(self.tcfg.checkpoint_dir, f'checkpoint_{tag}.pt')
        ...
        checkpoint = {
            ...
            'trainer_config': asdict(self.tcfg),
            'dataset_config': asdict(self.dcfg),
